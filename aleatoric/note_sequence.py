...
from aleatoric.utils import validate_sequence_of_type, validate_type


SequenceType = TypeVar('SequenceType')
SequencedType = TypeVar('SequencedType')


class SequenceBase(Generic[SequencedType, SequenceType]):
    def __init__(self, to_add: SequencedType):
        # Support constructing with empty note_list
        self.sequenced_type_list = to_add or []
        self.index = 0

    @staticmethod
    def _get_list(sequence_name, sequence):
        sequenced_type_list = None

        try:
            validate_type(sequence_name, sequence, SequenceType)
            sequenced_type_list = sequence.sequenced_type_list
        except ValueError:
            pass

        if not sequenced_type_list:
            try:
                validate_sequence_of_type(sequence_name, sequence, SequencedType)
                sequenced_type_list = sequence
            except ValueError:
                pass
                return None

        return sequenced_type_list

    # Manage list
    def append(self, elem: SequencedType) -> SequenceType:
        # validate_type('elem', elem, SequencedType)
        self.sequenced_type_list.append(elem)
        return self

    def extend(self, to_add: Union[SequencedType, List[SequencedType], SequenceType]) -> SequenceType:
        sequenced_type_list = None

        try:
            # validate_type('to_add', to_add, SequenceType)
            sequenced_type_list = to_add.sequenced_type_list
            self.sequenced_type_list.extend(sequenced_type_list)
        except ValueError:
            pass

        if not sequenced_type_list:
            try:
                # validate_sequence_of_type('to_add', to_add, SequencedType)
                sequenced_type_list = to_add
                self.sequenced_type_list.extend(sequenced_type_list)
            except ValueError:
                pass
        ...
        if not sequenced_type_list:
            try:
                # validate_type('to_add', to_add, SequencedType)
                self.sequenced_type_list.append(to_add)
            except ValueError:
                raise ValueError((f'Arg `to_add` must be a {SequencedType}, {SequenceType} or List[{SequencedType}], '
                                  f'arg: {to_add} type: {type(to_add)}'))

        return self

    def __add__(self, to_add: Union[SequencedType, List[SequencedType], SequenceType]) -> SequenceType:
        return self.extend(to_add)

    def __lshift__(self, to_add: Union[SequencedType, List[SequencedType], SequenceType]) -> SequenceType:
        return self.extend(to_add)

    def insert(self, index: int, to_add: Union[SequencedType, List[SequencedType], SequenceType]) -> SequenceType:
        validate_type('index', index, int)

        try:
            # validate_type('to_add', to_add, Note)
            self.sequenced_type_list.insert(index, to_add)
            return self
        except ValueError:
            pass

        sequenced_type_list = SequenceBase._get_list('to_add', to_add)
        if not sequenced_type_list:
            raise ValueError((f'Arg `to_add` must be a {SequencedType}, {SequenceType} or List[{SequencedType}], '
                              f'arg: {to_add} type: {type(to_add)}'))
        for elem in sequenced_type_list:
            self.sequenced_type_list.insert(index, elem)
            # Necessary to insert in the same order as input, rather than reverse order from the input
            index += 1

        return self

    def remove(self, to_remove: Union[SequencedType, List[SequencedType], SequenceType]) -> SequenceType:
        try:
            # validate_type('to_remove', to_remove, Note)
            self.sequenced_type_list.remove(to_remove)
            return self
        except ValueError:
            pass

        if not SequenceBase._get_list('to_remove', to_remove):
            raise ValueError((f'Arg `to_remove` must be a {SequencedType}, {SequenceType} or List[{SequencedType}], '
                              f'arg: {to_remove} type: {type(to_remove)}'))

        for elem in to_remove:
            self.sequenced_type_list.remove(elem)
    ...
    def __len__(self) -> int:
        return len(self.sequenced_type_list)

    def __getitem__(self, index: int) -> SequencedType:
        validate_type('index', index, int)
        if abs(index) >= len(self.sequenced_type_list):
            raise ValueError((f'`index` out of range index: {index} '
                              f'len(note_list): {len(self.sequenced_type_list)}'))
        return self.sequenced_type_list[index]

    def __iter__(self) -> SequenceType:
    ...
    def __next__(self) -> SequencedType:
        ...
        """
        if self.index == len(self.sequenced_type_list):
            raise StopIteration
        elem = self.sequenced_type_list[self.index]
        # perf_attrs comes from the NoteSequence if present, otherwise from the Note
        self.index += 1
        return SequencedType.__class__.copy(elem)

    def __eq__(self, other: SequenceType) -> bool:
        if not other or len(self) != len(other):
            return False
        return all([self.sequenced_type_list[i] == other.sequenced_type_list[i]
                    for i in range(len(self.sequenced_type_list))])
    # /Manage iter / slice


class NoteSequence(SequenceBase[Note, 'NoteSequence']):
    ...
    def __init__(self, to_add: Union[List[Note], 'NoteSequence']):
        ...
        #
        # self.index = 0
        super(NoteSequence, self).__init__(to_add)
        self.note_list = self.sequenced_type_list
