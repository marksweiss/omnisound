...
class Track(object):
    ...
    """A Track is a container of Measures that supports adding, removing and modifying Measures.
       ...
       sequence of notes rendered by one Player in the final Performance. Can be constructed empty,
       with a single Measure or a list of Measures, and optional PerformanceAttributes. PerformanceAttributes
       and all other Note attributes when set will be applied to all Measures in the Track, and therefore to all
       Notes in each Measure in the Track.
    """
    ...
    def extend(self, to_add: Union[Measure, List[Measure]]) -> 'Track':
        try:
    ...
    def __add__(self, to_add: Union[Measure, List[Measure]]) -> 'Track':
        return self.extend(to_add)

    def __lshift__(self, to_add: Union[Measure, List[Measure]]) -> 'Track':
        return self.extend(to_add)

    def insert(self, index: int, to_add: Union[Measure, List[Measure]]) -> 'Track':
    ...
    def remove(self, to_remove: Union[Measure, List[Measure]]) -> 'Track':
        try:
    ...
    def copy(source_track: 'Track') -> 'Track':
        ...
        new_track = Track(measure_list=measure_list, instrument=source_track.track_instrument,
